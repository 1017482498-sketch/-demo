    ...
    <div className="flex flex-col h-full bg-[#001838] relative overflow-hidden">
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-4 pb-8 z-50 bg-gradient-to-t from-[#001838] via-[#001838]/95 to-transparent flex space-x-3 items-end">
        <button 
          ...
          className="flex-1 bg-white/5 backdrop-blur-md text-white border border-[#d4af37]/40 py-3.5 rounded-2xl font-black text-sm active:bg-white/10 transition-all"
        >
          了解详情
        </button>
        <button className="flex-[2] relative group overflow-hidden bg-gradient-to-b from-[#f5e6ab] via-[#d4af37] to-[#b08e4f] text-[#001838] py-3.5 rounded-2xl font-black text-sm shadow-[0_10px_40px_rgba(212,175,55,0.3)] active:scale-[0.98] transition-all">
...
const TimelineNode: React.FC<{ pos: string, label: string, sub: string, active?: boolean, icon: React.ReactNode }> = ({ pos, label, sub, active, icon }) => (
  <div className={`absolute ${pos} flex flex-col items-center -rotate-[12deg] z-20`}>
    <div className={`
      ...
      ${active ? 'w-9 h-9 bg-white scale-125 shadow-[0_0_20px_rgba(212,175,55,1)]' : 'w-7 h-7 bg-[#d4af37] shadow-[0_0_12px_rgba(212,175,55,0.4)]'}
