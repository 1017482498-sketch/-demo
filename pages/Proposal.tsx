  ...
  return (
    <div className="flex flex-col min-h-full bg-[#f8fafc]">
      ...
      <header className="px-4 h-14 flex items-center bg-white sticky top-0 z-[60] border-b border-gray-100">
        ...
        <h1 className="text-lg font-bold text-gray-800 absolute left-1/2 -translate-x-1/2 pointer-events-none">保险建议书</h1>
      </header>
      ...
      <div className="px-4 pt-4 pb-2">
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-white border border-gray-200 rounded-xl py-3 pl-10 pr-4 text-sm focus:outline-none focus:border-blue-400 transition-colors shadow-sm"
          />
        </div>
      </div>
      ...
      <div className="px-4 py-4 space-y-4">
        {proposals.map((item) => (
          <div key={item.id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-50 flex flex-col space-y-4">
            <div className="space-y-3">
              <h3 className="text-lg font-bold text-gray-800 tracking-tight">{item.title}</h3>
        ...
        <div className="py-10 text-center">
          <p className="text-sm text-gray-300 font-medium">没有更多了</p>
        </div>
      </div>
    </div>
  );
